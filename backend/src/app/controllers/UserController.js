...
class UserController {
  ...
  async update(req, res) {
    ...
    if (oldPassword && (await user.checkPassword(oldPassword)) && !password) {
      return res.status(401).json({ Erro: 'Nova senha não informada!' });
    }

    if (password && oldPassword && password === oldPassword) {
      return res
        .status(401)
        .json({ Erro: 'A nova senha nãso pode ser igual a antiga!' });
    }
